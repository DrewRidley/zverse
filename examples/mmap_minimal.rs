...
use zverse::{Error, PersistentZVerse, ZVerseConfig};
...
fn main() -> Result<(), Error> {
    ...
    for i in 0..100_000 {
        ...
        let value = format!("value:{}", std::random::random::<i32>());
        db.put(&key, &value)?;
    }
    ...
    println!(
        "Inserted 100 items in {:?} ({:.2} items/sec)",
        insert_duration,
    ...
    for i in 0..100 {
        let key = format!("perf:key:{}", i);
        let _: Option<String> = db2
            .get(&key, None)?
            .map(|v: Vec<u8>| String::from_utf8(v).unwrap());
    }
    let read_duration = start.elapsed();
    println!(
        "Read 100 items in {:?} ({:.2} items/sec)",
        read_duration,
